on run argv
    ...
    tell application "BBEdit"
        ...
        if item 1 of rootChoice is "Choose a main .tex file…" then
            set rootPath to POSIX path of (choose file with prompt "Select the main .tex document (the file with documentclass).")
        end if
        set engines to {"Inherit — use main-file / project defaults", "pdflatex — standard LaTeX and publisher templates", "xelatex — system fonts, fontspec, Unicode", "lualatex — modern Unicode and Lua-based packages", "tectonic — automatic package downloads; requires Tectonic"}
        ...
        repeat with n from 2 to 5
            if item 1 of engineChoice is item n of engines then set engineName to item (n - 1) of {"pdflatex", "xelatex", "lualatex", "tectonic"}
        end repeat
